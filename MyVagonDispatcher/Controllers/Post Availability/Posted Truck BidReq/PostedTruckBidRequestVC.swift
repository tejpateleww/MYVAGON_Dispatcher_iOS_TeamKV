...
class PostedTruckBidRequestVC: BaseViewController {
    ...
    func setUpData(){
        self.SetUpTable()
        self.lblCompnyName.text = SingletonClass.sharedInstance.DispatcherProfileData?.companyName ?? ""
        self.lblId.text = "#\(BidsData?.postedTruck?.id ?? Int())"
        self.lblAmount.text = Currency + (BidsData?.postedTruck?.bidAmount ?? "")
    ...
    @IBAction func btnMatchFoundClick(_ sender: Any) {
        ...
        controller.driverId = BidsData?.postedTruck?.driverId ?? ""
