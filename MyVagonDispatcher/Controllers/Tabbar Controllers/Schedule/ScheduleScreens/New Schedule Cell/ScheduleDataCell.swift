...
class ScheduleDataCell: UITableViewCell {
    ...
    @IBOutlet weak var btnAssignDriver: UIButton!
    @IBOutlet weak var btnEditDriver: UIButton!
    @IBOutlet weak var driverHeightConstant: NSLayoutConstraint!
    
    var arrLocations : [MyLoadsNewLocation] = []
    var tblData :NewMyLoadData?
    var tblHeight:((CGFloat)->())?
    var btnMatchTapCousure :(() -> ())?
    var btnDeleteTap :(() -> ())?
    var btnAssignDriverTap :(() -> ())?
    ...
    //MARK: - Custom method
    func setupUI(){
        self.bringSubviewToFront(btnAssignDriver)
    ...
    func setPostedTruck(lodeData: NewMyLoadsPostedTruck?){
        setDriver(lodeData?.driverDetail?.name ?? "", status: MyLoadesStatus.completed.Name)
        if (lodeData?.bookingInfo?.locations?.count ?? 0) != 0{
            lblCompanyName.text = lodeData?.bookingInfo?.shipperDetails?.companyName ?? ""
            ...
            arrLocations = lodeData?.bookingInfo?.locations ?? []
            relodeTable()
            btnMatches.isHidden = true
            btnDelete.isHidden = true
        }else{
            lblCompanyName.text = SingletonClass.sharedInstance.DispatcherProfileData?.companyName ?? ""
...
//            if let truck = SingletonClass.sharedInstance.TruckTypeList?.first(where: {$0.id == tblData?.postedTruck?.truckTypeId}){
...
//            }
    ...
    func setData(data : NewLoadData?){
        lblTonMiles.isHidden = false
        lblCompanyName.text = data?.shipperDetails?.companyName ?? ""
        lblAmount.text = Currency + "\(data?.amount ?? "")"
        lblAmount.isHidden = false
        lblLoadId.text = "#\(data?.id ?? 0)"
        lblTonMiles.text =   "\(data?.totalWeight ?? ""), \(data?.distance ?? "") Km"
        setDriver(data?.driverDetail?.name ?? "", status: data?.status ?? "",pickUpDate: data?.pickupTime ?? "",currentDate: data?.currentTime ?? "")
        btnMatches.isHidden = true
    ...
    func setDriver(_ driverName:String,status: String,pickUpDate: String = "",currentDate: String = ""){
        if driverName != ""{
            self.btnAssignDriver.setTitle(driverName, for: .normal)
            if status == MyLoadesStatus.pending.Name || status == MyLoadesStatus.scheduled.Name{
                self.btnAssignDriver.isUserInteractionEnabled = true
                self.btnEditDriver.isHidden = false
            }else{
                self.btnEditDriver.isHidden = true
                self.btnAssignDriver.isUserInteractionEnabled = false
            }
            driverHeightConstant.constant = 30
        }else{
            self.btnAssignDriver.setTitle("assigndriver".localized, for: .normal)
            driverHeightConstant.constant = (status != MyLoadesStatus.canceled.Name) ? 30 : 0
            self.btnAssignDriver.isUserInteractionEnabled = true
            self.btnEditDriver.isHidden = true
        }
        self.btnAssignDriver.layer.borderWidth = 1
        self.btnAssignDriver.setTitleColor(hexStringToUIColor(hex: "#9B51E0"), for: .normal)
        self.btnAssignDriver.layer.borderColor = hexStringToUIColor(hex: "#9B51E0").cgColor
        self.btnAssignDriver.layer.cornerRadius = 10
        self.btnAssignDriver.clipsToBounds = true
        
        self.btnEditDriver.layer.borderWidth = 1
        self.btnEditDriver.setTitleColor(hexStringToUIColor(hex: "#9B51E0"), for: .normal)
        self.btnEditDriver.layer.borderColor = hexStringToUIColor(hex: "#9B51E0").cgColor
        self.btnEditDriver.layer.cornerRadius = 10
        self.btnEditDriver.clipsToBounds = true
    }
    ...
    @IBAction func btnAssignDriverClick(_ sender: Any) {
        if let obj = btnAssignDriverTap{
            obj()
        }
    }
}
...
extension ScheduleDataCell : UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if tblData?.type == MyLoadType.PostedTruck.Name{
            if ((tblData?.postedTruck?.bookingInfo?.locations?.count ?? 0) == 0) {
                return 2
            }
        }
