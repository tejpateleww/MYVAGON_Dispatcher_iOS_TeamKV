    ...
    @IBAction func btnLocTrackingAction(_ sender: Any) {
        ...
        if(self.currentState == .EntrouteTo){
            if(findDistance()){
                ...
                Utilities.ShowAlertOfInfo(OfMessage: "\("You're not at the".localized) \(self.arrLocations[self.currentLocation - 1]).")
            }
        }else if(self.currentState == .StartLoading || self.currentState == .StartUnloding || self.currentState == .LodingUnloding){
...
extension TrackingVC : UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if(indexPath.row == currentLocation - 1){
            ...
            }else if(self.currentState == .ArrivedAt || self.currentState == .StartLoading || self.currentState == .CompleteLoading) {
...
extension TrackingVC {
    ...
    func CallAPIForCompleteTrip() {
        ...
        reqModel.booking_id = "\(self.TripDetails?.id ?? 0)"
        reqModel.location_id = "\(self.TripDetails?.trucks?.locations?[self.currentLocation - 1].id ?? 0)"
        reqModel.pod_image = self.TripDetails?.podURL ?? ""
