...
class ScheduleDetailVC: BaseViewController {
    ...
    @IBOutlet weak var noteViewHeightConstant: NSLayoutConstraint!
    @IBOutlet weak var tblDriverDetail: UITableView!
    @IBOutlet weak var tblDriverHeightConstent: NSLayoutConstraint!
    @IBOutlet weak var lblSeparator: UILabel!
    
    let schedualDetailViewModel = SchedualDetailViewModel()
    var LoadDetails : MyLoadsNewBid?
    var arrTypes:[(TruckType?,Bool)] = []
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?){
        if let info = object, let collObj = info as? UITableView{
            ...
            if collObj == self.tblDriverDetail{
                self.tblDriverHeightConstent.constant = tblDriverDetail.contentSize.height
            }
        }
    }
    ...
    // MARK: - Custom methods
    func setupUI(){
        if LoadDetails?.driverDetails?.id == nil{
            self.btnStartTrip.setTitle("assigndriver".localized, for: .normal)
            self.lblSeparator.isHidden = true
        }else{
            self.btnStartTrip.setTitle("changeDriver".localized, for: .normal)
        ...
        self.tblDriverDetail.separatorStyle = .none
        ...
        self.tblDriverDetail.delegate = self
        self.tblDriverDetail.dataSource = self
        tblDriverDetail.addObserver(self, forKeyPath: "contentSize", options: .new, context: nil)
        ...
        self.tblDriverDetail.register(UINib(nibName: "DriversTableViewCell", bundle: nil), forCellReuseIdentifier: "DriversTableViewCell")
        arrTypes.removeAll()
        self.LoadDetails?.vehicles?.forEach({ element in
            arrTypes.append((element.truckType,true))
        })
        ColTypes.reloadData()
    ...
    func reloadAfterStartTrip(){
        self.btnEnlarge.isHidden = true//LoadDetails?.status == MyLoadesStatus.inprocess.Name ? false : true
    }
    ...
    func SetValue() {
        ...
        let DateOfPickup = "\(data?.pickupDate ?? "") \(data?.pickupTimeTo ?? "")"
        ...
        if(data?.status == MyLoadesStatus.scheduled.Name){
            ...
            if(strHour.contains("-")){
                ...
                let components = cal.dateComponents([.hour], from: CurrentDate, to: dateToCompare)
                let diff = components.hour!
                var dateString = ""
                if diff / 24 != 0{
                    let day = diff / 24
                    dateString = (day == 1) ? "\(day) \("day".localized)" : "\(day) \("days".localized)"
                }
                if diff % 24 != 0{
                    let hours = diff % 24
                    let str = (hours == 1) ? "\(hours) \("hour".localized)" : "\(hours) \("hours".localized)"
                    dateString = "\(dateString) \(str)"
                }
                self.lblDaysToGo.text = "\("hoursLeft".localized) \(dateString) \("to go".localized)"
            }
        }
        ...
        self.noteViewHeightConstant.constant = ((data?.trucks?.note ?? "") == "") ? 0 : 40
        ...
        self.lblPrice.text = Currency + "\(amount ?? "")"
    ...
    func callWebServiceToAssignDriver(driver:DispaterDriver?){
        ...
        viewModel.callApiToAssignDriver(reqModel: reqModel) {
            self.LoadDetails?.driverDetails = driver
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "RefreshViewForPostTruck"), object: nil, userInfo: nil)
            self.setupUI()
            self.SetValue()
            self.tblDriverDetail.reloadData()
        }
    }
    ...
    @IBAction func btnStartTripClick(_ sender: themeButton) {
        if sender.titleLabel?.text == TripStatus.UploadPOD.Name.localized {
            ...
            SingletonClass.sharedInstance.getAvailableDriverList(bookingId: "\(self.LoadDetails?.id ?? 0)") { data in
                ...
                if filterData?.count ?? 0 > 0 {
                    ...
                    controller.clickEdit = { (driver) in
                        ...
                        self.callWebServiceToAssignDriver(driver: driver)
                    }
                    //                    controller.dismiss = {
...
extension ScheduleDetailVC: UITableViewDelegate,UITableViewDataSource{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if tableView == tblMainData{
            return LoadDetails?.trucks?.locations?.count ?? 0
        }else{
            return (LoadDetails?.driverDetails?.id != nil) ? 1 : 0
        }
    }
...
extension ScheduleDetailVC: UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if collectionView == ColTypes{
            return CGSize(width: ((arrTypes[indexPath.row].0?.getName().capitalized ?? "").sizeOfString(usingFont: CustomFont.PoppinsMedium.returnFont(14)).width) + 30, height: ColTypes.frame.size.height - 10)
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == ColTypes{
            ...
            cell.lblTypes.text = arrTypes[indexPath.row].0?.getName()
...
extension ScheduleDetailVC{
    ...
    func CallAPIForAcceptPayment() {
        ...
        reqModel.driver_id = "\(LoadDetails?.driverDetails?.id ?? 0)"
        reqModel.booking_id = "\(self.LoadDetails?.id ?? 0)"
