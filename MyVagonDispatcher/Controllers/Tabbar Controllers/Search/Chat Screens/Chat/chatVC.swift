...
class ChatVC: BaseViewController {
    ...
    @IBAction func btnSendClick(_ sender: Any) {
        if tvWriteMessage.text.trimmingCharacters(in: .whitespacesAndNewlines) != "" {
            let param = ["sender_id" : "\(SingletonClass.sharedInstance.DispatcherProfileData?.id ?? 0)",
...
extension ChatVC: UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if(!self.isTblReload){
            ...
            if(self.arrData.count > 0){
                if(arrData[indexPath.row].senderId == SingletonClass.sharedInstance.DispatcherProfileData?.id){
