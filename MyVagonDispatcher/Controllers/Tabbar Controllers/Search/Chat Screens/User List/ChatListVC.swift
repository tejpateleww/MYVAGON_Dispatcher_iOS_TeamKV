...
class ChatListVC: BaseViewController {
    ...
    func setupUI(){
        self.setNavigationBarInViewController(controller: self, naviColor: .clear, naviTitle: "Chat".localized, leftImage: NavItemsLeft.back.value, rightImages: [NavItemsRight.contactus.value], isTranslucent: true, IsChatScreenLabel:true,NumberOfChatCount: "5")
    ...
    @objc func reloadFilter() {
        ...
        if searchText.isEmpty == false {
            ...
            self.arrFilterData =  self.arrData.filter ({ ($0.name)?.lowercased().contains(searchText.lowercased()) ?? Bool()})
        }
        self.tblChatList.reloadData()
    }
    ...
    func registerNib(){
        ...
        self.tblChatList.register(nib, forCellReuseIdentifier: NoDataTableViewCell.className)
        let nib2 = UINib(nibName: NotiShimmerCell.className, bundle: nil)
        self.tblChatList.register(nib2, forCellReuseIdentifier: NotiShimmerCell.className)
    }
...
extension ChatListVC:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(!self.isTblReload){
            ...
            if(isSearched ? self.arrFilterData.count > 0 : self.arrData.count > 0){
                ...
                cell.lblMessage.text = (isSearched) ? self.arrFilterData[indexPath.row].message : self.arrData[indexPath.row].message?.html2String
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if(isSearched ? self.arrFilterData.count > 0 : self.arrData.count > 0){
            let id :Int = (isSearched) ? self.arrFilterData[indexPath.row].id ?? 0 : self.arrData[indexPath.row].id ?? 0
            let name :String = (isSearched) ? self.arrFilterData[indexPath.row].name ?? "" : self.arrData[indexPath.row].name ?? ""
            ...
            controller.shipperID = AppDelegate.shared.shipperIdForChat
            controller.shipperName = AppDelegate.shared.shipperNameForChat
            // Uncomment to implement allow chat.
//            if isSearched{
//                controller.allowChat = (self.arrFilterData[indexPath.row].chatDisable == 0) ? false : true
//            }else{
//                controller.allowChat = (self.arrData[indexPath.row].chatDisable == 0) ? false : true
//            }
            AppDelegate.shared.shipperProfileForChat = AppDelegate.shared.shipperProfileForChat
            self.navigationController?.pushViewController(controller, animated: true)
        }
...
extension ChatListVC{
    func callChatUserListAPI() {
        ...
        reqModel.driver_id = "\(SingletonClass.sharedInstance.DispatcherProfileData?.id ?? 0)"
