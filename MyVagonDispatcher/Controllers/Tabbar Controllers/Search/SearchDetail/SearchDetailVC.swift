...
class SearchDetailVC: BaseViewController {
    ...
    func setValue() {
        ...
        LoadDetails?.vehicles?.forEach({ element in
...
//        })
    ...
    func setUpUI(){
        ...
        noteViewHeight.constant = ((LoadDetails?.trucks?.note ?? "") == "") ? 0 : 40
...
//        lblDeadHead.text = "\(Double(LoadDetails?.trucks?.locations?.first?.deadhead ?? "") ?? 0.0) Km Deadhead"
    ...
    func openReloadView(strTitle : String,isFromRelode: Bool = false,driverID:String = ""){
        ...
        controller.isFromReload = isFromRelode
        ...
        controller.reloadClick = {
            ...
            controller.driverId = driverID
            controller.bookingId = "\(self.LoadDetails?.id ?? 0)"
    ...
    func bookNow(){
        ...
        controller.showDriverSelection = true
        var height = 260
        if controller.availabilityId != "0"{
            height = 200
            controller.showDriverSelection = false
        }
...
extension SearchDetailVC: UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if collectionView == ColTypes{
            return CGSize(width: ((arrTypes[indexPath.row].0?.getName().capitalized)?.sizeOfString(usingFont: CustomFont.PoppinsMedium.returnFont(14)).width ?? 0.0) + 30
