...
//        let _ = self.btnSelection.map{$0.isSelected = false}
    ...
    func setupUI(){
        ...
        self.selectedBtnUIChanges(Selected: true, Btn: btnSelection.first ?? UIButton())
    ...
    func setUpNevigetionBar(){
        if isFromEdit{
            if isEditEnable {
                ...
                setNavigationBarInViewController(controller: self, naviColor: .clear, naviTitle: "Tractor_details_title".localized, leftImage: NavItemsLeft.back.value, rightImages:(editData?.isAssign == 0) ? [NavItemsRight.editProfile.value] : [], isTranslucent: true, ShowShadow: true)
            }
        }else{
            self.setNavigationBarInViewController(controller: self, naviColor: UIColor.white, naviTitle: "add_tractor".localized, leftImage: NavItemsLeft.back.value, rightImages: [], isTranslucent: true)
        }
    }
    
    func setupData(){
        txtLicencePlateNumber.delegate = self
    ...
    func setUpEditData(){
        ...
        self.setFuleType(fuleType: editData?.fuelType ?? Tabselect.Diesel.rawValue)
        self.selectedBrandID = Int(editData?.brandId?.id ?? 0)
        ...
        self.txtLicencePlateNumber.text = editData?.registrationNo
        self.arrImages = editData?.tractorImage?.components(separatedBy: ",") ?? []
        setArray()
        self.collectionImages.reloadData()
    }
...
extension TractorDetailVC : UITextFieldDelegate{
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if textField == txtLicencePlateNumber {
...
extension TractorDetailVC{
    ...
    func saveTractor(){
        let reqModel = AddTractorReqModel()
        reqModel.brandId = "\(self.selectedBrandID)"
        reqModel.fuelType = tabTypeSelection
        reqModel.registrationNo = txtLicencePlateNumber.text
        self.editTractorDetailViewModel.tractorVc = self
        reqModel.tractorImage = arrImages.map({$0}).joined(separator: ",")
        if isFromEdit{
            reqModel.tractorId = "\(editData?.id ?? 0)"
            self.editTractorDetailViewModel.callWebServiceToUpdateTractor(reqModel: reqModel)
        }else{
            self.editTractorDetailViewModel.callWebServiceToAddTractor(reqModel: reqModel)
        }
    }
