...
class SingletonClass: NSObject{
    ...
    var DispetcherRegisterData = RegistrationData()
    var DispatcherProfileData : DispatcherLoginData?
    
    func clearSingletonClassForRegister() {
        DispetcherRegisterData = RegistrationData()
    ...
    func getDriverList(complition:@escaping ([DispaterDriver]?)->()){
        Utilities.showHud()
    ...
    func getAvailableDriverList(bookingId:String,complition:@escaping ([DispaterDriver]?)->()){
        let reqModel = AvailableDriverList()
        reqModel.bookingId = bookingId
        Utilities.showHud()
        WebServiceSubClass.getAvailableDriverList(reqModel: reqModel, completion: { (status, apiMessage, response, error) in
            Utilities.hideHud()
            if status {
                complition(response?.data)
            }else{
