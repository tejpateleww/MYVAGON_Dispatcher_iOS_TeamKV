...
//          loadingView.layer.cornerRadius = 10
    ...
    class func AlwaysAllowPermission(currentVC:UIViewController){
        ...
        if Utilities.checkLocationPermission(){
            let alertController = UIAlertController(title: "My Vagoon does not have access to your location while in the background. To enable access, tap Settings > Location and select Always", message: "", preferredStyle: .alert)
...
//    }
    ...
    class func checkLocationPermission() -> Bool{
         ...
         if LocationStatus == .notDetermined {
             appDel.locationManager.setLocationManager()
             return false
         }
        return true
     }
