...
class AddTruckViewModel{
    ...
    func callWebServiceToAddTruck(reqModel:DispatcherAddTruckReqModel){
        Utilities.ShowLoaderButtonInButton(Button: TruckDetail?.btnSave ?? themeButton(), vc: TruckDetail ?? UIViewController())
        WebServiceSubClass.dispatcherAddTruck(reqModel: reqModel) {(status, apiMessage, response, error) in
            Utilities.HideLoaderButtonInButton(Button: self.TruckDetail?.btnSave ?? themeButton(), vc: self.TruckDetail ?? UIViewController())
            if status{
                self.TruckDetail?.truckAdded()
    ...
    func callWebServiceToUpdateTruck(reqModel:DispatcherAddTruckReqModel){
        Utilities.ShowLoaderButtonInButton(Button: TruckDetail?.btnSave ?? themeButton(), vc: TruckDetail ?? UIViewController())
        WebServiceSubClass.dispatcherUpdateTruck(reqModel: reqModel) {(status, apiMessage, response, error) in
            Utilities.HideLoaderButtonInButton(Button: self.TruckDetail?.btnSave ?? themeButton(), vc: self.TruckDetail ?? UIViewController())
            if status{
                self.TruckDetail?.truckAdded()
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) {
