    ...
    func Login(ReqModel:LoginReqModel){
        ...
        WebServiceSubClass.Login(reqModel: ReqModel, completion: { (status, apiMessage, response, error) in
            ...
            if status{
                ...
                SingletonClass.sharedInstance.DispatcherProfileData = response?.data
                ...
                if response?.data?.type == LoginType.freelancer.rawValue {
                    ...
                    appDel.NavigateToHome()
                }else if response?.data?.type == LoginType.dispatcher.rawValue {
                    UserDefault.setValue(LoginType.dispatcher.rawValue, forKey: UserDefaultsKey.LoginUserType.rawValue)
                    appDel.NavigateToHome()
                }
            } else {
